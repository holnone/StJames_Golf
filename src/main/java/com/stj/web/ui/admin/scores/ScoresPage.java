...
import com.stj.web.wicket.renderer.CourseRenderer;
import com.stj.web.wicket.renderer.TeamMatchRenderer;
...
public class ScoresPage extends AdminBasePage {
	...
	DropDownChoice<Course> courseDropDown;
	...
	public ScoresPage(Week week, TeamMatch match) {
	...
	private void renderContent() {
		...
		for (int i = 0; i < matches.length; i++) {
			...
			getMatchContainer().add(getEditLink("match" + matches[i] + "EditLink", "match" + matches[i]));
		...
		getMatchContainer().add(buildCourseDropDown("matchCourse", "matchCourse"));
	...
	private DropDownChoice<Course> buildCourseDropDown(String id, String property) {
		if (courseDropDown == null) {
			courseDropDown = new DropDownChoice<Course>(id, new PropertyModel<Course>(this, property), courseChoices, new CourseRenderer());
			courseDropDown.setOutputMarkupId(true);
			courseDropDown.add(new AjaxFormComponentUpdatingBehavior("onchange") {
				private static final long serialVersionUID = 1L;

				@Override
				protected void onUpdate(AjaxRequestTarget target) {
					setMatchCourse(courseDropDown.getConvertedInput());
					if (getMatchCourse() != null) {
						if (getSelectedTeamMatch() != null) {
							getSelectedTeamMatch().setCourse(getMatchCourse());
							getSelectedTeamMatch().updateChildren();
							leagueService.saveTeamMatch(getSelectedTeamMatch());
						}
						resetMatchups();
					}
	...
	private DropDownChoice<TeamMatch> getTeamMatchDropDown() {
		if (teamMatchDropDown == null) {
			teamMatchDropDown = new DropDownChoice<TeamMatch>("teamMatchChoice", new PropertyModel<TeamMatch>(this, "selectedTeamMatch"),
					matchChoices, new TeamMatchRenderer());
